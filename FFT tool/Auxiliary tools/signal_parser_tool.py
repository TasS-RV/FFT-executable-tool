"""
Signal parser and format converter for cogging torque analysis.

Developed by Tasin Sayed (2025), 

This module takes in the csv file containing the columns `X`, `CH1` (torque signal), `CH2` (sine), and `CH3` (cosine) - from the oscilloscipe hooked up to either a torque ripple or cogging rig.
The workflow mirrors the original `cogging.py` reference originally developed by Phill Philippou (2025), but is
packaged into discrete functions, with modifiable parameters for scaling and looking at graphs, alongside various filters:

- `tan_angle_to_mechanical_angle` uses the arctan2 method to generate a smooth
  mechanical angle, remove DC offsets, and emit the Combined_book-style CSV.

- `data_plotter` provides optional XY and polar visualisations of the processed
  data so results can be inspected immediately.

- `zero_crossing_angle_conversion` applies a sine-only zero-crossing detector
  to recover angle steps when arctan2 cannot be used or when a half-cycle model
  is preferred.

The script expects input CSVs in the standard lab layout (header row, unit row
that is skipped, and numeric data thereafter). 

Outputs are written with the columns `position4`, `CH4`, and `X` so they drop directly into downstream
tools such as `integrated_FFT_GUI.py` that rely on the Combined_book4 format of the csv. 
This essentially outputs the torque signal variation (as a voltage) with against increments in mechanical
angle (with the big assumption, that between each 'step' - or zero crossing, motor spins at constant speed).
"""
...
if __name__ == "__main__":
    ...
    input_file_arctan = "./Data/Syntec motor.csv"  # "./Data/085_2U_1132001_original_rotor_and_stator 1.csv"
    output_file_arctan =  "./Data/Syntec_motor_arctan_method.csv"  # "./Data/085_2U_1132001_original_arctan_method.csv"
    ...
    input_file_zc = "./Data/Syntec motor.csv"  # "./Data/085_2U_1132001_original_rotor_and_stator 1.csv"
    output_file_zc = "./Data/Syntec_motor_zero_cross_sine_only.csv"  # "./Data/085_2U_1132001_zero_cross_sine_only.csv"
